...
static AFX_EXTENSION_MODULE TGcutDllsDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("TGcutDlls.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(TGcutDllsDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(TGcutDllsDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("TGcutDlls.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(TGcutDllsDLL);
	}
	return 1;   // OK
}
